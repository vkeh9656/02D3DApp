...
void CGameEdu01::OnInit()
{
	...
	// Note: 적 미사일 설정
	m_EnemyBulletProperty.fBulletVelocity = 0.01f;	// 미사일 속력
	m_EnemyBulletProperty.fScale = 0.9f;			// 미사일 크기
...
void CGameEdu01::OnUpdate()
{
	...
	m_dwElapsedTime = dwCurTime - OldTime; // 1프레임마다의 짧은 시간 간격
	OldTime = dwCurTime;
	D3DXMATRIX matWorld;
	...
	for (i = 0; i < m_nEnemyIndex; i++)
	{
		if (m_Enemy[i].nLife)
		{
			...
			{
				m_Enemy[i].nLife = 0;
				m_nEnemyCount--;		// 적 캐릭터의 개수 0이 되면 종료
				continue;
			}
