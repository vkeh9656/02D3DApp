...
void CGround::Create(LPDIRECT3DDEVICE9 pd3dDevice, int nRow, int nCol, float fSize)
{
	...
	for (int z = 0; z <= m_nRow; z++) // 행 인덱스
	{
		for (int x = 0; x <= m_nCol; x++)  // 열 인덱스
		{
			pGroundVertex[nIndex].vPos.x = vPos0.x + (fSize * x);
			pGroundVertex[nIndex].vPos.y = 0.0f;			// y는 고정
	...
	// Note: 인덱스 버퍼 생성
	WORD* pIndex = new WORD[m_dwNumIndices]; // 인덱스는 WORD 단위로 할당되기 때문에 그에 맞게 메모리 생성
	nIndex = 0;
